...
```
Vercel (Vite SPA) ── HTTPS /api/* ──▶ Render Docker (FastAPI + Whisper + OpenCV + MediaPipe)
                                           │
                                           ▼
...
| `backend/Dockerfile` | API image (installs ffmpeg, libgl1, libglib2.0, libgomp1 for Whisper/OpenCV/MediaPipe) |
| `backend/.dockerignore` | Keeps build context small, excludes `.env` secrets |
| `frontend/vercel.json` | Vercel build settings + SPA rewrites (`/any-route → /index.html`) |
| `frontend/.env.example` | `VITE_API_URL` template |
| `backend/pyproject.toml` | Backend metadata + pinned deps |

### Step 0 — MongoDB Atlas (do this first, both deploys depend on it)
1. [cloud.mongodb.com](https://cloud.mongodb.com) → Build a Database → **M0 (Free)** → pick a region near your Render region (e.g. Oregon / us-west).
2. **Database Access** → Add Database User (password auth), e.g. `interviewsense_app` + a strong password.
3. **Network Access** → Add IP Address → **Allow access from anywhere `0.0.0.0/0`** (Render does not publish static egress IPs).
4. **Connect → Drivers** → copy the connection string and append the db name:
   `mongodb+srv://<user>:<pass>@<cluster>.mongodb.net/interviewsense?retryWrites=true&w=majority`

### Step 1 — Render: deploy the API
1. [dashboard.render.com](https://dashboard.render.com) → **New → Blueprint** → pick this repo (branch `master`). Render reads `render.yaml`.
2. In the Blueprint review screen it will ask for `MONGODB_URL` (marked `sync: false`) — paste the Atlas connection string from Step 0.
3. `JWT_SECRET` is auto-generated by Render (`generateValue: true`) — nothing to do.
4. Apply → first build takes **5–10 min** (Whisper + OpenCV + MediaPipe wheels are large). Wait for `Uvicorn running` in the log.
5. Verify `https://<your-render-url>/api/health` returns `"database": "connected"`.
6. Copy the service URL, e.g. `https://interviewsense-api.onrender.com`.

> Free plan note: instances sleep after 15 min idle → first request takes ~50s to wake, plus ~1–2 min if Whisper must re-download the model. Starter plan stays warm.

### Step 2 — Vercel: deploy the frontend
1. [vercel.com](https://vercel.com) → **Add New → Project** → import this repo.
2. **Root Directory: `frontend`** ← crucial, the repo root has no package.json.
3. Framework preset auto-detects Vite (from `vercel.json`); Build `npm run build`, Output `dist`.
4. **Environment Variables** (add for Production + Preview + Development):
   | Key | Value |
   |---|---|
   | `VITE_API_URL` | `https://<your-render-url>` (no trailing slash) |

> `VITE_API_URL` must be set **before** the first build — Vite inlines it into the bundle at build time. Set it, then Deploy (or redeploy if already deployed).

### Step 3 — Close the loop: CORS
1. Note your Vercel production domain, e.g. `https://interviewsense.vercel.app`.
2. Render → `interviewsense-api` → **Environment** → edit `CORS_ORIGINS`:
   `http://localhost:5173,http://127.0.0.1:5173,https://interviewsense.vercel.app`
3. Save — Render redeploys automatically. Done.

### Environment Variable Matrix

| Variable | Platform | Value | Notes |
|---|---|---|---|
| `MONGODB_URL` | Render → Backend | `mongodb+srv://user:pass@cluster.mongodb.net/interviewsense?retryWrites=true&w=majority` | Atlas string from Step 0. **Secret.** Asked for during Blueprint review. |
| `MONGODB_DB_NAME` | Render → Backend | `interviewsense` | Pre-set in render.yaml |
| `JWT_SECRET` | Render → Backend | *(auto-generated)* | `generateValue: true` in render.yaml. **Secret.** |
| `JWT_ALGORITHM` | Render → Backend | `HS256` | Pre-set |
| `JWT_EXPIRE_MINUTES` | Render → Backend | `1440` | Pre-set (24h sessions) |
| `WHISPER_MODEL` | Render → Backend | `base` (or `tiny` for speed) | Pre-set |
| `QUESTION_COUNT` | Render → Backend | `10` | Pre-set |
| `CORS_ORIGINS` | Render → Backend | localhost values + `https://<vercel-domain>` | Edit after Vercel deploy (Step 3) |
| `PORT` | Render → Backend | `8000` | Pre-set; Render also injects its own |
| `VITE_API_URL` | Vercel → Frontend | `https://<render-url>` | **Only frontend key.** Not secret (compiled into JS). |

### Post-deploy checklist
- [ ] Render `/api/health` → `{"status":"ok","database":"connected"}`
- [ ] Vercel app loads; hard-refresh on `/login` works (SPA rewrite)
- [ ] Register → Dashboard → Setup → run a full interview (camera + mic need HTTPS — Vercel/Render provide it automatically)
- [ ] Results dashboard shows Overall/Content/Delivery after completion
- [ ] First registered user gets the `admin` role
- [ ] `CORS_ORIGINS` updated with the real Vercel domain

### Local production sanity check (before deploying)
```bash
cd frontend && npm run build && npm run preview   # http://127.0.0.1:4173
# put your Atlas URL in backend/.env → MONGODB_URL=mongodb+srv://...
```
